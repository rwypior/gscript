...
namespace gscript
{
	...
	ParseResult ParserArrayAccessor::parse(StringIteratorRange itrange)
	{
		...
		if (!beginResult.isOk())
			return ParseResult(ParseResult::STATUS_T::S_FATAL, ParserEntity::StringIteratorRange(beginResult.result.begin, beginResult.result.end));

		std::string::iterator end = beginResult.result.end;
		...
		if (this->indexType & ParserArrayAccessor::INDEX_TYPE_T::IT_REQUIRED || this->indexType & ParserArrayAccessor::INDEX_TYPE_T::IT_OPTIONAL)
		{
			...
			else if (this->indexType & ParserArrayAccessor::IT_LITERAL)
			{
				...
				assert(false && "Array specifier must contain either IT_REQUIRED or IT_STATEMENT bitflags");
			...
			if (statementResult.isOk())
			{
				...
				return ParseResult(ParseResult::STATUS_T::S_FATAL, ParserEntity::StringIteratorRange(beginResult.result.begin, end));
		}

		ParseResult endResult = Util::Char::parse(StringIteratorRange(end, itrange.end), ParserArrayAccessor::KW_ARRAY_ACCESSOR_END);

		if (!endResult.isOk())
			return ParseResult(ParseResult::STATUS_T::S_FATAL, ParserEntity::StringIteratorRange(endResult.result.begin, endResult.result.end));

		return ParseResult(ParseResult::STATUS_T::S_OK, ParserEntity::StringIteratorRange(beginResult.result.begin, endResult.result.end));
	}
}