...
namespace gscript
{
	...
	ParseResult ParserVar::parse(StringIteratorRange itrange)
	{
		...
		std::string::iterator end = nameResult.result.end;
		...
		return ParseResult(ParseResult::STATUS_T::S_OK, ParserEntity::StringIteratorRange(nameResult.result.begin, end));
	}
}